...
namespace Math123
{
    public static class NumberEnumerable
    {
        ...
        public static IEnumerable<long> MakeNumber(IEnumerable<IEnumerable<long>> digits)
        {
            var d = digits.ToList();
            var count = d.Count;

            return MakeNumberReq(d.Select((r, i) => new Tuple<long, IEnumerable<long>>(TenPow(count - i - 1), r)));
        }
        ...
        private static IEnumerable<long> MakeNumberReq(IEnumerable<Tuple<long, IEnumerable<long>>> digits)
        {
            ...
            if (rest.Any())
            {
                ...
                {
                    foreach (var restPart in MakeNumberReq(rest))
                    {
